...
- run the server: =promnesia serve=
  ...
  - [optional] autostart the server with =promnesia service install=

    This sets it up to autostart and run in the background.
    
    Management commands: =promnesia service {install,start,stop,restart,status,logs}=
...
- [optional] setup MIME handler to jump to files straight from the extension
  ...
  See a short [[https://karlicoss.github.io/promnesia-demos/jump_to_editor.webm][20s demo]]. The extension now supports native file opening:
  tries VS Code, then system default, with [[https://github.com/karlicoss/open-in-editor#readme][open-in-editor]] as fallback.
